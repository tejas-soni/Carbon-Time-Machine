...
export const ShareCard: React.FC<ShareCardProps> = ({ result }) => {
  ...
  const appLink = 'https://carbontimemachine.tejassoni.in';
...
#CarbonTimeMachine #ClimateAwareness`;
  ...
  const twitterText = `I visited my Carbon Time Machine! ⏳🌲\nPattern: ${archetype}\nFuture Risk: ${futureMood}\nShifted Future: ${shiftedFutureMood}\n\nThe future bends when the pattern changes.`;
  ...
  return (
    <div className="card" style={{ marginTop: '24px' }}>
      ...
      <div style={{ marginTop: '12px', display: 'flex', gap: '8px', justifyContent: 'center' }}>
        <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(twitterText)}&url=${encodeURIComponent(appLink)}&hashtags=CarbonTimeMachine`} target="_blank" rel="noreferrer" className="btn btn-outline" style={{ padding: '6px 12px', fontSize: '0.85rem', flex: 1, textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          Post
        </a>
        <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(appLink)}`} target="_blank" rel="noreferrer" className="btn btn-outline" style={{ padding: '6px 12px', fontSize: '0.85rem', flex: 1, textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
